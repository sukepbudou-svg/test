...
### 買い目選出方式（ET+STベース: 2連単2点＋3連単2点）
- **軸選出の手順**:
  1. 展示タイム速い順でランク（値が小さいほど速い）
  2. 展示ST≥0.18の艇はET1/ET2候補から除外（STデータなしは除外しない）
  3. 1号艇がST除外されておらず、かつ最速候補との差が0.05秒以内 → 1号艇をET1に昇格（1コースアドバンテージ考慮）
- **2連単2点**: ET1-ET2 と ET2-ET1（両方向）
- **3連単2点**: ET1-ET2-3着 と ET2-ET1-3着（40〜250倍・範囲外なら次点3着候補へ）
  - 3着候補: 脅威艇（A1選手/前付け/ST速のある艇）優先 → ET中位（3位以降）順
  - 脅威艇2艇以上なら3点目追加
- **コスト**: 1レース最大400円（2連単100円×2 ＋ 3連単100円×2）
- **1号艇弱い時の動作**: ST≥0.18なら自動的に候補から落ちるため外艇が頭になる
- 6号艇1着のみ除外（1コース最遠のため）
