...
- 参戦ライン: **6P以上**
- 見送りでも予想を出力（データ収集のため）
- 出力は現在2点（主役艇固定×展開2シナリオ）← 将来3点以上に増やす可能性あり

### 買い目選出方式（主役艇1艇固定×2シナリオ） ← 点数変更時はここも更新すること
1. **主役艇（hero）選出**: 3〜6号艇から複合スコア最高の1艇
   - ST速さ (40%) + ET速さ (30%) + グレード (20%) + まくり実績 (10%)
2. **シナリオA（まくり差し）**: hero-1-残り最速 の3連単
3. **シナリオB（まくり）**: hero-直外の艇-残り最速 の3連単

> **重要**: 倍率（オッズ）は買い目選出には一切使わない。
> 選出後に表示するだけ。倍率のばらつきは外枠狙いの構造上の結果。
> `min_roi`パラメータは定義済みだが現在未使用（将来の期待回収率フィルター用）。
