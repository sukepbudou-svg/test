...
### 買い目選出方式（EV上位2点＋1-2着flip）
- 80倍超の組み合わせからEV上位2点を選出し、各組み合わせの1-2着入れ替えを追加（計最大4点）
- コスト: 1レース最大400円（flipが既存選択と重複する場合は3点以下になることあり）
- 例: EV上位が「3-2-5」「4-2-6」→ flip追加で「2-3-5」「2-4-6」→ 計4点
- flipは80倍未満でも追加（1-2着のどちらが先でも拾えるよう）
- EV = LightGBM＋エージェント合算モデル確率 × ライブオッズ（市場が過小評価する組み合わせを自動選出）
