...
# ─── 各指標の重み（基準スコアを構成する8指標の合計が1.0になるよう正規化）───
# 旧版は合計0.80止まりで、平均的な選手のベーススコアが1.0ではなく0.80になる
# バグがあった（グレード・気配補正等の乗算基準がズレる）。
W_NATIONAL_WIN  = 0.17 / 0.80
W_LOCAL_WIN     = 0.13 / 0.80
W_MOTOR_2RATE   = 0.10 / 0.80
W_NATIONAL_2    = 0.11 / 0.80
W_LOCAL_2       = 0.10 / 0.80  # 当地2連率
W_BOAT_2        = 0.05 / 0.80  # 艇2連率
W_NATIONAL_3    = 0.05 / 0.80  # 全国3連率（近似値）
W_RECENT_FORM   = 0.09 / 0.80  # 直近調子（過去10走の着順）
W_ST            = 0.10  # 展示ST重み（乗算補正の係数、上記の基準スコアとは別枠）
W_EXHTIME       = 0.10  # 展示タイムランク重み（同上）
